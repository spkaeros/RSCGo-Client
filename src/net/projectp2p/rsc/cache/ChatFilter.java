...
public class ChatFilter {
	...
	public static String filterString(String s) {
		...
		method372(ac);
		method382(ac);
		for (int i = 0; i < aStringArray558.length; i++) {
			for (int j = -1; (j = s.indexOf(aStringArray558[i], j + 1)) != -1;) {
				char ac1[] = aStringArray558[i].toCharArray();
	...
	public static void loadBadData(DataEncryption dataEncryption) {
		int i = dataEncryption.get4ByteInt();
	...
	public static void loadFragmentData(DataEncryption dataEncryption) {
		anIntArray551 = new int[dataEncryption.get4ByteInt()];
		for (int i = 0; i < anIntArray551.length; i++)
			anIntArray551[i] = dataEncryption.get2ByteInt();
	...
	public static void method367(DataEncryption dataEncryption, char ac[][],
		...
		for (int i = 0; i < ac.length; i++) {
			...
			ac[i] = ac1;
			byte abyte1[][] = new byte[dataEncryption.get4ByteInt()][2];
			for (int k = 0; k < abyte1.length; k++) {
				abyte1[k][0] = (byte) dataEncryption.getByte();
				abyte1[k][1] = (byte) dataEncryption.getByte();
			}
