package net.projectp2p.rsc.terrian;

import java.io.File;
import java.io.IOException;
import java.nio.ByteBuffer;

import net.projectp2p.rsc.cache.Archive;
import net.projectp2p.rsc.cache.Data;
import net.projectp2p.rsc.cache.Utils;
...
import net.projectp2p.rsc.various.DataConversions;
import net.projectp2p.rsc.various.Sector;
...
public class World {
	...
	public void populateSection(int i, int j, int k) {
		...
		if (tiles != null)
			for (int x = 0; x < 96; x++) {
				for (int y = 0; y < 96; y++) {
					tiles[areaX + x][areaY + y] = modelAdjacency[x][y];
				}
			}
	}
