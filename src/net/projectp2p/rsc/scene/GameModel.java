package net.projectp2p.rsc.scene;
...
public class GameModel {
	...
	public GameModel(String path) {
		...
		int verticeCount = readBase64(data);
		int faceCount = readBase64(data);
		allocate(verticeCount, faceCount);
		faces = new int[faceCount][];
		for (int j3 = 0; j3 < verticeCount; j3++) {
	...
	private void computeBounds() {
		...
		for (int face = 0; face < faceCount; face++) {
			...
			int x1;
			int x2 = x1 = vertexTransformedX[vertex];
			int y1;
			int y2 = y1 = vertexTransformedY[vertex];
			int z1;
			int z2 = z1 = vertexTransformedZ[vertex];
			for (int j = 0; j < count; j++) {
				vertex = vertice[j];
				if (vertexTransformedX[vertex] < x1)
					x1 = vertexTransformedX[vertex];
				else if (vertexTransformedX[vertex] > x2)
					x2 = vertexTransformedX[vertex];
				if (vertexTransformedY[vertex] < y1)
					y1 = vertexTransformedY[vertex];
				else if (vertexTransformedY[vertex] > y2)
					y2 = vertexTransformedY[vertex];
				if (vertexTransformedZ[vertex] < z1)
					z1 = vertexTransformedZ[vertex];
				else if (vertexTransformedZ[vertex] > z2)
					z2 = vertexTransformedZ[vertex];
			}
	...
	public int makeFace(int i, int ai[], int j, int k) {
	...
	public void merge(GameModel[] pieces, int count, boolean transState) {
		...
		for (int l = 0; l < count; l++) {
			faceCount += pieces[l].faceCount;
			verticeCount += pieces[l].verticeCount;
		}
			...
			for (int sourceFace = 0; sourceFace < source.faceCount; sourceFace++) {
				...
				if (transState)
					if (count > 1) {
						...
						faces[destFace][0] = i1;
						for (int i2 = 0; i2 < source.faces[sourceFace].length; i2++)
							faces[destFace][i2 + 1] = source.faces[sourceFace][i2];

					} else {
						faces[destFace] = new int[source.faces[sourceFace].length];
						for (int j2 = 0; j2 < source.faces[sourceFace].length; j2++)
							faces[destFace][j2] = source.faces[sourceFace][j2];
	...
	public void project(int cameraX, int cameraY, int cameraZ, int cameraPitch,
		...
		for (int vertex = 0; vertex < verticeCount; vertex++) {
			int x = vertexTransformedX[vertex] - cameraX;
			int y = vertexTransformedY[vertex] - cameraY;
			int z = vertexTransformedZ[vertex] - cameraZ;
			if (cameraYaw != 0) {
	...
	public int readBase64(byte[] buff) {
		for (; buff[dataOffset] == 10 || buff[dataOffset] == 13; dataOffset++)
			;
	...
	public GameModel[] split(int i, int j, int pieceDX, int pieceDZ, int rows,
		...
		for (int face = 0; face < faceCount; face++) {
			int sumX = 0;
			int sumZ = 0;
			int verticeCount = faceVerticeCount[face];
			int[] vertice = faceVertices[face];
			for (int i4 = 0; i4 < verticeCount; i4++) {
				sumX += vertexX[vertice[i4]];
				sumZ += vertexZ[vertice[i4]];
			}

			int piece = sumX / (verticeCount * pieceDX)
					+ (sumZ / (verticeCount * pieceDZ)) * rows;
			pieceVerticeCount[piece] += verticeCount;
		...
		for (int face = 0; face < faceCount; face++) {
			...
			int piece = sumX / (verticeCount * pieceDX)
					+ (sumZ / (verticeCount * pieceDZ)) * rows;
			copyLighting(pieces[piece], vertice, verticeCount, face);
		}
