...
public abstract class GameManager<Delegate_T extends ImplementationDelegate>
	...
	public void drawLoadingScreen() {
		...
		// gfx.drawImage(this.loadingImage, x, y,
		// this.delegate.getContainerImpl());
		x += 2;
		y += 90;
		gfx.setColor(new Color(132, 132, 132));
		gfx.drawRect(x - 2, y - 2, 280, 23);
		gfx.setColor(new Color(198, 198, 198));
		drawString(gfx, "http://rscproelium.net/", HELVETICA,
				x + 138, y + 30);
	}
	...
	protected void drawString(Graphics g, String s, Font font, int x, int y) {
		...
		g.setFont(font);
		g.drawString(s, x - fontmetrics.stringWidth(s) / 2,
	...
	public boolean keyDown(boolean shift, boolean ctrl, boolean action,
		...
		this.keys.put(Integer.valueOf(key), Boolean.valueOf(true));
		...
		if ((key == 8) && (this.inputText.length() > 0))
			this.inputText = this.inputText.substring(0,
	...
	protected Socket makeSocket(String address, int port) throws IOException {
		return mudclient.makeSecureSocket(address, port);
	}
