...
public class DataFileDecrypter {
	...
	public static byte[] loadCachedData(String entry, byte[] arc) {
		...
		for (int cur = 0; cur < cnt; cur++) {
			int name_ = (arc[(cur * 10 + 2)] & 0xFF) * 16777216
					+ (arc[(cur * 10 + 3)] & 0xFF) * 65536
					+ (arc[(cur * 10 + 4)] & 0xFF) * 256
					+ (arc[(cur * 10 + 5)] & 0xFF);
			int decmp_len = (arc[(cur * 10 + 6)] & 0xFF) * 65536
					+ (arc[(cur * 10 + 7)] & 0xFF) * 256
					+ (arc[(cur * 10 + 8)] & 0xFF);
			int ptr = (arc[(cur * 10 + 9)] & 0xFF) * 65536
					+ (arc[(cur * 10 + 10)] & 0xFF) * 256
					+ (arc[(cur * 10 + 11)] & 0xFF);
			if (name_ == name) {
	...
	public static int unpackData(byte[] abyte0, int j, byte[] abyte1, int k,
			int i, String filename) {
		return unpackData(abyte0, j, abyte1, k, i);
	}
}
