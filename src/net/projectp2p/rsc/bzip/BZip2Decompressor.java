package net.projectp2p.rsc.bzip;

public class BZip2Decompressor {
	private static void createDecodeTables(int limit[], int base[], int perm[],
			byte length[], int minLen, int maxLen, int alphaSize) {
		int pp = 0;
		...
		for (int j1 = 0; j1 < MAX_CODE_LEN; j1++)
			base[j1] = 0;

		for (int k1 = 0; k1 < alphaSize; k1++)
			base[length[k1] + 1]++;

		for (int l1 = 1; l1 < MAX_CODE_LEN; l1++)
			base[l1] += base[l1 - 1];

		for (int i2 = 0; i2 < MAX_CODE_LEN; i2++)
			limit[i2] = 0;
	...
	private static int getBits(int i, BZip2BlockEntry blockEntry) {
		...
		do {
			if (blockEntry.anInt577 >= i) {
				int k = blockEntry.anInt576 >> blockEntry.anInt577 - i
						& (1 << i) - 1;
				blockEntry.anInt577 -= i;
				j = k;
				break;
			}
			blockEntry.anInt576 = blockEntry.anInt576 << 8
					| blockEntry.inputBuffer[blockEntry.offset] & 0xff;
	...
	private static void readBlock(BZip2BlockEntry blockEntry) {
		...
		while (flag19) {
			...
			for (int t = 0; t < groupCount; t++) {
				...
				createDecodeTables(blockEntry.limit[t], blockEntry.base[t],
						blockEntry.perm[t], blockEntry.len[t], minLen, maxLen,
						alphaSize);
			...
			int zvec;
			byte thebit;
			for (zvec = getBits(zn, blockEntry); zvec > limit_zt[zn]; zvec = zvec << 1
					| thebit) {
				zn++;
			...
			for (int nextSym = perm_zt[zvec - base_zt[zn]]; nextSym != eob;)
				if (nextSym == 0 || nextSym == 1) {
					...
					do {
						...
						int zvec2;
						byte thebit2;
						for (zvec2 = getBits(zn2, blockEntry); zvec2 > limit_zt[zn2]; zvec2 = zvec2 << 1
								| thebit2) {
							zn2++;
					...
					byte tmp;
					if (j11 < 16) {
						int j10 = blockEntry.anIntArray593[0]; // should be
																// 0!!!!
						...
						if (blockEntry.anIntArray593[0] == 0) {
							...
							for (int k9 = 15; k9 >= 0; k9--) {
								for (int l9 = 15; l9 >= 0; l9--) {
									blockEntry.yy[i10] = blockEntry.yy[blockEntry.anIntArray593[k9]
											+ l9];
									i10--;
								}
					...
					int j8;
					byte byte11;
					for (j8 = getBits(k7, blockEntry); j8 > limit_zt[k7]; j8 = j8 << 1
							| byte11) {
						k7++;
			...
			flag19 = blockEntry.anInt584 == blockEntry.anInt601 + 1
					&& blockEntry.anInt574 == 0;
		}
	}
